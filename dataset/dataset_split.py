...
def split_dataset(rgb_dir, depth_dir, output_dir, train_ratio=0.8, val_ratio=0.2, test_ratio=0.0, seed=42):
    ...
    for split, names in splits.items():
        # 创建新结构：train/images/train, train/labels/train, train/depth 等
        for folder in ["images", "labels", "depth"]:
            out_dir = Path(output_dir) / split / folder
            os.makedirs(out_dir, exist_ok=True)

        for name in names:
            # 图像
            shutil.copy(Path(rgb_dir) / f"{name}.png", Path(output_dir) / split / "images" / f"{name}.png")
            # 标签
            xml_file = Path(rgb_dir) / f"{name}.xml"
            if xml_file.exists():
                convert_robndbox(xml_file, Path(output_dir) / split / "labels", rgb_dir)
            # 深度图
            depth_name = f"depth_{name.split('_', 1)[1]}.png"
            depth_file = Path(depth_dir) / depth_name
            if depth_file.exists():
                shutil.copy(depth_file, Path(output_dir) / split / "depth" / depth_name)
...
if __name__ == "__main__":
    split_dataset(
        rgb_dir="./tomato_leaf/rgb0508_anno",
        depth_dir="./tomato_leaf/depth0508_anno",
