...
__all__ = "Detect", "Segment", "Pose", "Classify","OBBDepth" "OBB", "RTDETRDecoder", "v10Detect", "YOLOEDetect", "YOLOESegment"
...
class OBBDepth(Detect):
    """YOLOv8 Depth head for depth estimation models."""
    """The input should from the largest to the smallest."""
    def __init__(self, nc=80, ne=1, ch=()):
        """Initialize the YOLO model attributes such as the number of masks, prototypes, and the convolution layers."""
        super().__init__(nc, ch)
        self.ne = ne  # number of extra parameters
        c4 = max(ch[0] // 4, self.ne)
        self.cv4 = nn.ModuleList(nn.Sequential(Conv(c, c4, 3), Conv(c4, c4, 3), nn.Conv2d(c4, self.ne, 1)) for c in ch)
    
        self.stride = torch.zeros(self.nl)  # strides computed during build
        c5 = max(16, ch[0] // 4)  # channels
        self.cv5 = nn.ModuleList(
            nn.Sequential(Conv(c, c5, 3)) for c in ch
        )
        self.cv6 = nn.ModuleList(nn.Sequential(Conv(c5, c5, 3)) for _ in ch)
        self.cv7 = nn.ModuleList(nn.Sequential(nn.Conv2d(c5, 1, 1)) for _ in ch)
    ...
    def forward(self, x):
        ...
        # print(f"x_copy[0].shape:{x_copy[0].shape}")
        y = Detect.forward(self, x_copy)
        # No fusion
        for i in range(self.nl):
            x[i] = self.cv5[i](x[i])
            x[i] = self.cv6[i](x[i])
            x[i] = self.cv7[i](x[i])  # Output: B×1×H×W
        ...
        #         x[i - 1] = self.cv5[i-1](x[i - 1]) + up_feat
        #     feat = 
            

            # if i ==0:
            #     feat = self.cv5[i](x[i])
            #     feat = self.cv6[i](feat)
            #     up_feat = F.interpolate(feat, size=x[i + 1].shape[2:], mode='nearest')
            #     x[i + 1] = self.cv5[i+1](x[i + 1]) + up_feat
            # elif i == self.nl - 1:
            #     feat = self.cv6[i](feat)
            ...
            # depth = self.cv7[i](feat)        # 输出为 B×1×H×W
            # # print(depth.shape)
        ...
        if self.training:  # 训练模式下，返回三个 tensor list
            return y, angle, x

        # 推理模式下
        # y 是 tuple: (preds, raw_x)
        # preds.shape: (B, no, N) → permute→ (B, N, no)
        preds, raw_x = y

        # 拼接 angle 和 depth（只选 x[0] 的深度图作为最终输出）
        # angle.shape: (B, 1, N)
        angle_cat = angle.permute(0, 2, 1)  # (B, N, 1)

        # preds: (B, no, N) → permute(0, 2, 1) → (B, N, no)
        preds = preds.permute(0, 2, 1)

        # 组合 preds + angle → (B, N, no+1)
        preds = torch.cat([preds, angle_cat], dim=-1)
        return (preds, raw_x, x[0])  # B x N x (no+1), raw_x for decode, x[0] as depth
