...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            ...
            tmp = _forward(torch.zeros(1, ch, s, s))
...
class OBBDModel(DetectionModel):
    """YOLO Oriented Bounding Box (OBB) model."""

    def __init__(self, cfg="yolo11n-obb-depth.yaml", ch=3, nc=None, verbose=True):
        """
        Initialize YOLO OBB model with given config and parameters.
        ...
        self._real_forward = self.forward
        self.forward = lambda x: [torch.zeros(1, 1, 80, 80)] * 3  # dummy 结构骗过 Detect
        super().__init__(cfg=cfg, ch=ch, nc=nc, verbose=verbose)
        self.forward = self._real_forward
        self._init_stride_manually(ch)

    def init_criterion(self):
        """Initialize the loss criterion for the model."""
        return v8OBBLoss(self)
    
    def _init_stride_manually(self, ch=3):
            s = 256
            m = self.model[-1]
            if isinstance(m, Detect):
                with torch.no_grad():
                    ...
                    # 如果你 forward 返回 (det, angle, depth)
                    if isinstance(out, (tuple, list)) and isinstance(out[0], (list, torch.Tensor)):
                        out = out[0]

                    m.stride = torch.tensor([s / o.shape[-2] for o in out])
                    self.stride = m.stride
                    m.bias_init()
            else:
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            {Detect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBBDepth, OBB, ImagePoolingAttn, v10Detect}
        ):
            args.append([ch[x] for x in f])
            if m is Segment or m is YOLOESegment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, YOLOEDetect, Segment, YOLOESegment, Pose, OBBDepth, OBB}:
                m.legacy = legacy
