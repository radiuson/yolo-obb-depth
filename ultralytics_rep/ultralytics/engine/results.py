...
class Results(SimpleClass, DataExportMixin):
    ...
    Attributes:
        ...
        obb_depth (OBBDepth | None): Depth information for oriented bounding boxes.
    ...
    def __init__(
        self, orig_img, path, names, boxes=None, masks=None, probs=None, keypoints=None, obb=None, obb_depth=None, speed=None
    ) -> None:
        """
        ...
        self.obb_depth = obb_depth
        self.speed = speed if speed is not None else {"preprocess": None, "inference": None, "postprocess": None}
        self.names = names
        self.path = path
        self.save_dir = None
        self._keys = "boxes", "masks", "probs", "keypoints", "obb", "obb_depth"
    ...
    def update(self, boxes=None, masks=None, probs=None, obb=None, obb_depth=None, keypoints=None):
        """
    ...
    def plot(
        ...
        print("obb_depth:", type(self.obb_depth))
        print("hasattr depth:", hasattr(self.obb_depth, 'depth'))
        print("depth content:", self.obb_depth.depth if hasattr(self.obb_depth, 'depth') else None)

        if self.obb_depth is not None and hasattr(self.obb_depth, 'obb_depth') and self.obb_depth.depth:
            print("Depth map is available, overlaying on the image.")
            import cv2
            import numpy as np
            depth_map = self.obb_depth.depth[0].detach().cpu().squeeze().numpy()
            depth_map = cv2.resize(depth_map, (annotator.im.shape[1], annotator.im.shape[0]))
            ...
            alpha = 0.5
            annotator.im = cv2.addWeighted(annotator.im, 1 - alpha, color_map, alpha, 0)
...
class OBBDepth(BaseTensor):

    def __init__(self, boxes: torch.Tensor | np.ndarray, angle: torch.Tensor | np.ndarray, depth: torch.Tensor | np.ndarray, orig_shape) -> None:
        """
        Initialize an OBBDepth instance with oriented bounding box data and original image shape.
        """

        if boxes.ndim == 1:
            boxes = boxes[None, :]
        assert boxes.shape[-1] >= 7, f"Expected at least 7 values in boxes (xywhr + conf + cls), got {boxes.shape[-1]}"
        super().__init__(boxes, orig_shape)

        self.angle = angle
        self.depth = depth
        self.orig_shape = orig_shape
        self.is_track = boxes.shape[-1] == 8
    ...
    @property
    def inv_depth(self):
        """Return the inverse depth maps."""
        return [1.0 / (d + 1e-6) for d in self.depth]
    
    @property
    def depth_map(self):
        """Return the predicted depth (or inverse depth)."""
        return self.depth
